...
-----
  ...
  python ewars_scraper.py --self-test

  # 2. Do a real run (needs open internet -- run on YOUR machine, not a sandbox):
  python ewars_scraper.py --years 2020 2021 2022 2023 2024 2025
...
PDF_DIR = "ewars_pdfs"                # where downloaded PDFs are cached
...
def write_csvs(national_rows, district_rows):
    if national_rows:
        ...
        cols = (["year", "week", "reporting_rate_pct", "sites_reporting",
                ...
                + ["age_top_districts"])
        with open("ewars_national_weekly.csv", "w", newline="") as f:
            w = csv.DictWriter(f, fieldnames=cols)
            w.writeheader()
            w.writerows(national_rows)
        print(f"\nWrote ewars_national_weekly.csv ({len(national_rows)} weeks)")
    if district_rows:
        with open("ewars_age_districts_long.csv", "w", newline="") as f:
            w = csv.DictWriter(f, fieldnames=["year", "week", "district", "age_cases"])
            w.writeheader()
            w.writerows(district_rows)
        print(f"Wrote ewars_age_districts_long.csv ({len(district_rows)} rows)")
